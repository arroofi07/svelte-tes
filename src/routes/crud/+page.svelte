<script lang="ts">
	...
	const deletePeople = () => {
		peoples = peoples.filter(
			(p) => p.name !== newPerson.name || p.umur !== newPerson.umur || p.email !== newPerson.email
		);
		clearInput();
	};
...
<div class="m-10">
	<div class="">
		...
		<input type="text" class="border-2 border-black" bind:value={newPerson.name} />
	</div>
	<div class="">
		<label for="">Umur :</label>
		<input type="text" class="border-2 border-black" bind:value={newPerson.umur} />
	</div>
	<div class="">
		<label for="">Email :</label>
		<input type="text" class="border-2 border-black" bind:value={newPerson.email} />
	</div>
</div>
